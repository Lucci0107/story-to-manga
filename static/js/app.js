  ...
  function initWorkspace(initial) {
    ...
    function renderStoryboard() {
      ...
      const pageMarkup = pages.map(function (page, pageIndex) {
        ...
        const fallbackNotice = page.composition_fallback_notice
          ? '<div class="form-notice" role="status"><span class="notice-mark">i</span><p>' + escapeHtml(page.composition_fallback_notice) + '</p></div>'
          : "";
    ...
    function renderQA() {
      ...
      const compositionQuality = result?.composition_quality;
      const compositionSummary = compositionQuality && compositionQuality.pages?.length ? '<p class="qa-composition-score">漫画構成 v3：読みやすさ ' + escapeHtml(compositionQuality.average_readability ?? "-") + ' / 視覚階層 ' + escapeHtml(compositionQuality.average_visual_hierarchy ?? "-") + ' / 構成指摘 ' + escapeHtml(compositionQuality.issue_count ?? 0) + '件</p>' : '';
